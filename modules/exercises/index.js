  ...
  if (response.toLowerCase()==='y'){
    if (i===0){
      ...
      for (let j = 0; j < testTitles.length; j++){
        let avg = averageForTest(scores[j]) //Call 'averageForTest' here. Pass in j and scores as arguments.
        console.log(`${testTitles[j]} test average = ${avg}%.`);
      }
    } else if (i===2){
      for (let j = 0; j<astronauts.length; j++){
        let avg = averageForStudent(scores[j]) //Call 'averageForStudent' here. Pass in j and scores as arguments.
